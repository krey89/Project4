...
//Kevin Reyes
//ii) each consumer has its own queue, and the producers can produce in either queue;
 
public class PPC2 {	//Solution 2
	...
	static long startMS;
	static long endMS;
	public PPC2(int bffrsize, int maxZPro, int maxZCon, int numMssgs){
   ...
   public static void main(String[] args) {     
	   ...
	   System.out.println("IN () SEPARATED BY COMMAS(,) WRITE 4 INTEGERS");
		...
		PPC2 sol2 = new PPC2(bffrsize,maxZPro,maxZCon,numMssgs);
...
public void runSimulation1(){
   ...
   class QManager{			
      ...
      public synchronized void produceItem(String data, String producerName){		//Produce Item
    	  if(startcounter == 0){
         ...
         while((currentSize == bffrsize)&&(currentSize2 == bffrsize)){
         ...
         if(currentSize < currentSize2){	//choose shortest buffer
      ...
      public synchronized String consumeItem(String consumerName){				//ConsumeItem for C1
    	  ...
    	  if(cyclecounter == (numMssgs*2)){
    		    ...
    		    System.out.println("End of Cycle 2");
    	  }
    	  notifyAll();
         try {
      ...
      public synchronized String consumeItem2(String consumerName){				//ConsumeItem for C2
     	 
    	  while(currentSize2 == 0){
            try {
               wait();
            } catch (InterruptedException e) {
               e.printStackTrace();
   ...
   class Consumer extends Thread{
      ...
      public void run() {
    	  while(true){
              if(getName().equals("C1")){	//choose consumer method
        		  q.consumeItem(getName());               

              }else{
        		  q.consumeItem2(getName());               
